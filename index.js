...
let phoneNumber = "6288989923014"
let owner = "6288989923014"
...
const {  state, saveCreds } = await useMultiFileAuthState(`./session`)
    ...
    const conn = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
        ...
        return {
            conversation: "Hi, saya Beltok Botz"
        }
      },
   ...
   if (pairingCode && !conn.authState.creds.registered) {
      if (useMobile) throw new Error('[!] Tidak dapat menggunakan API pairing code')
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("[!] Masukan nomor telepon anda untuk pairing code : (+62xxxx)")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`[!] Masukan nomor telepon anda untuk melakukan pairing code\nContoh (+62xxxxxxx : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`[!] Masukan nomor telepon anda untuk melakukan pairing code, Contoh (+62xxxxxxx : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`[!] Pairing kode anda : `)), chalk.black(chalk.white(code)))
      }, 3000)
   }
...
var budy = (typeof m.text == 'string' ? m.text : '')
        ...
        require("./hamzz")(conn, m, chatUpdate, store);
    } catch (err) {
        console.log(err);
    }
});
...
conn.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
        say('Beltok Xid', { font: 'shade', align: 'center', gradient: ['blue', 'green'] });
        	console.log(chalk.magenta(` `))
            console.log(chalk.green(require('./package').name+`, terhubung ke : ` + JSON.stringify(conn.user, null, 2)))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
